...
*/
public class AsyncSqlServiceVertxEBProxy implements AsyncSqlService {

  private Vertx _vertx;
  private String _address;
  private boolean closed;

  public AsyncSqlServiceVertxEBProxy(Vertx vertx, String address) {
    this._vertx = vertx;
  ...
  public void getConnection(Handler<AsyncResult<AsyncSqlConnection>> handler) {
    ...
    _vertx.eventBus().<AsyncSqlConnection>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        handler.handle(Future.failedFuture(res.cause()));
      } else {
        String addr = res.result().headers().get("proxyaddr");
        handler.handle(Future.succeededFuture(ProxyHelper.createProxy(AsyncSqlConnection.class, _vertx, addr)));
      }
    });
  }
