...
*/
public class PostgresqlServiceVertxProxyHandler extends ProxyHandler {

  private final Vertx vertx;
  private final PostgresqlService service;
  private final String address;

  public PostgresqlServiceVertxProxyHandler(Vertx vertx, PostgresqlService service, String address) {
    this.vertx = vertx;
      ...
      case "begin": {
        service.begin(res -> {
          if (res.failed()) {
            ...
            ProxyHelper.registerService(PostgresqlTransaction.class, vertx, res.result(), proxyAddress);
      ...
      case "take": {
        service.take(res -> {
          if (res.failed()) {
            ...
            String proxyAddress = UUID.randomUUID().toString();
            ProxyHelper.registerService(PostgresqlConnection.class, vertx, res.result(), proxyAddress);
            msg.reply(null, new DeliveryOptions().addHeader("proxyaddr", proxyAddress));
          }
        });
        break;
