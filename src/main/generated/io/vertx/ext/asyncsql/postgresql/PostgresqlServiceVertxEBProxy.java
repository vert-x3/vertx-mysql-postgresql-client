...
import io.vertx.ext.asyncsql.postgresql.PostgresqlConnection;
import io.vertx.core.json.JsonArray;
...
*/
public class PostgresqlServiceVertxEBProxy implements PostgresqlService {

  private Vertx _vertx;
  private String _address;
  private boolean closed;

  public PostgresqlServiceVertxEBProxy(Vertx vertx, String address) {
    this._vertx = vertx;
  ...
  public void begin(Handler<AsyncResult<PostgresqlTransaction>> transaction) {
    ...
    _vertx.eventBus().<PostgresqlTransaction>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        transaction.handle(Future.failedFuture(res.cause()));
      } else {
        String addr = res.result().headers().get("proxyaddr");
        transaction.handle(Future.succeededFuture(ProxyHelper.createProxy(PostgresqlTransaction.class, _vertx, addr)));
      }
    });
  }
  ...
  public void take(Handler<AsyncResult<PostgresqlConnection>> connection) {
    ...
    _vertx.eventBus().<PostgresqlConnection>send(_address, _json, _deliveryOptions, res -> {
      if (res.failed()) {
        connection.handle(Future.failedFuture(res.cause()));
      } else {
        String addr = res.result().headers().get("proxyaddr");
        connection.handle(Future.succeededFuture(ProxyHelper.createProxy(PostgresqlConnection.class, _vertx, addr)));
      }
    });
  }
