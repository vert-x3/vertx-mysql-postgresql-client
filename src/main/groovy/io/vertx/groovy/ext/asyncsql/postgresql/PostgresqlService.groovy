...
package io.vertx.groovy.ext.asyncsql.postgresql;
 ...
 */
@CompileStatic
public class PostgresqlService implements BaseSqlService,  DatabaseCommands {
  final def io.vertx.ext.asyncsql.postgresql.PostgresqlService delegate;
  public PostgresqlService(io.vertx.ext.asyncsql.postgresql.PostgresqlService delegate) {
  ...
  public static PostgresqlService create(Vertx vertx, Map<String, Object> config) {
    def ret= PostgresqlService.FACTORY.apply(io.vertx.ext.asyncsql.postgresql.PostgresqlService.create((io.vertx.core.Vertx)vertx.getDelegate(), config != null ? new io.vertx.core.json.JsonObject(config) : null));
    return ret;
  }
  public static PostgresqlService createEventBusProxy(Vertx vertx, String address) {
    def ret= PostgresqlService.FACTORY.apply(io.vertx.ext.asyncsql.postgresql.PostgresqlService.createEventBusProxy((io.vertx.core.Vertx)vertx.getDelegate(), address));
    return ret;
  }
  /**
   ...
   */
  public void begin(Handler<AsyncResult<PostgresqlTransaction>> transaction) {
    this.delegate.begin(new Handler<AsyncResult<io.vertx.ext.asyncsql.postgresql.PostgresqlTransaction>>() {
      public void handle(AsyncResult<io.vertx.ext.asyncsql.postgresql.PostgresqlTransaction> event) {
        AsyncResult<PostgresqlTransaction> f
        if (event.succeeded()) {
          f = InternalHelper.<PostgresqlTransaction>result(new PostgresqlTransaction(event.result()))
        } else {
          f = InternalHelper.<PostgresqlTransaction>failure(event.cause())
        }
   ...
   */
  public void take(Handler<AsyncResult<PostgresqlConnection>> connection) {
    this.delegate.take(new Handler<AsyncResult<io.vertx.ext.asyncsql.postgresql.PostgresqlConnection>>() {
      public void handle(AsyncResult<io.vertx.ext.asyncsql.postgresql.PostgresqlConnection> event) {
        AsyncResult<PostgresqlConnection> f
        if (event.succeeded()) {
          f = InternalHelper.<PostgresqlConnection>result(new PostgresqlConnection(event.result()))
        } else {
          f = InternalHelper.<PostgresqlConnection>failure(event.cause())
        }
        connection.handle(f)
      }
    });
  ...
  static final java.util.function.Function<io.vertx.ext.asyncsql.postgresql.PostgresqlService, PostgresqlService> FACTORY = io.vertx.lang.groovy.Factories.createFactory() {
    io.vertx.ext.asyncsql.postgresql.PostgresqlService arg -> new PostgresqlService(arg);
  };
}
